...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const antes = await prisma.unidadMedida.findFirst({ where: { id, empresaId } });
  ...
  if (parsed.data.nombre && parsed.data.nombre !== antes.nombre) {
    ...
    await prisma.insumo.updateMany({
      where: { unidadMedida: antes.nombre, empresaId },
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const unidad = await prisma.unidadMedida.findFirst({ where: { id, empresaId } });
  if (!unidad) return NextResponse.json({ error: "No encontrado" }, { status: 404 });

  const count = await prisma.insumo.count({ where: { unidadMedida: unidad.nombre, empresaId } });
  if (count > 0)
