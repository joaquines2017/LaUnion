...
| RNFS-005 | Instalar y configurar Fail2ban en ambos servidores: banear IPs con +3 intentos fallidos SSH en 5 min. | ALTA | ✅ Completado | Nuevo `scripts/setup-fail2ban.sh` (idempotente): instala `fail2ban` y escribe `/etc/fail2ban/jail.local` con jail `sshd` (`port=2190`, `maxretry=3`, `findtime=5m`, `bantime=1h`). `ignoreip` incluye `127.0.0.1/8 ::1 192.168.100.0/24` (toda la LAN) para evitar autobloqueos del admin o entre los propios servidores; sigue protegiendo ante intentos desde fuera de la LAN si en el futuro se expone SSH (junto con RNFS-001). Backend `auto` de fail2ban usa `/var/log/auth.log` (ya presente y rotando en ambos contenedores) sin configuración extra. Aplicado en `.190` y `.191` (commit `a9110ea`). Verificado en ambos: `systemctl is-active fail2ban` → `active`, `fail2ban-client status sshd` con jail cargado y `Currently banned: 0`, `maxretry/findtime/bantime` = `3/300/3600`. |
...
RNFS-005 (Fail2ban). Solo quedan diferidos al final RNFS-001 (firewall) y
...
- **2026-06-14**: Completado RNFS-005 (Fail2ban). Nuevo
  `scripts/setup-fail2ban.sh`: jail `sshd` con `port=2190`, `maxretry=3`,
  `findtime=5m`, `bantime=1h`, `ignoreip` cubre `192.168.100.0/24` para
  evitar autobloqueos en la LAN. Aplicado y verificado en `.190` y `.191`
  (commit `a9110ea`). Solo quedan diferidos RNFS-001 (firewall) y
  RNFB-001/002/003 (backups).
